...
function AdminHabitaciones() {
  ...
  const handleEdit = (habitacionId) => {
    setModoEdicion(true);
    const habitacion = data.find(
      (habitacion) => habitacion._id === habitacionId
    );
  ...
  if (tipos.length > 0) {
    return (
      <div>
        ...
        {modoCreacion ? (
          <AdminCreaHabitaciones
            handleCancelCreate={() => setModoCreacion(false)}
          />
        ) : (
