...
const postServicio = async (req,res) => {
    ...
    if (!nombre || !precio) {return res.status(400).send("Error. No se enviaron los datos necesarios para crear la servicio")};
...
const putServicio = async (req,res) => {
    const {id} = req.params;
    const {nombre, descripcion, precio} = req.body;
    if (!nombre || !precio) {return res.status(400).send("Error. No se enviaron los datos necesarios para actualizar")};
